			...
			if(header.length > 0){
				...
				if (countMenuParents !== 0) {
					...
					if (countMenuParents>1) {
						centerChild = Math.floor(countMenuParents / 2);
					} else {
						centerChild = 1;
					}
	...
	$(document).ready(function () {
		...
		if (!hasTouchMobile && !is_mobile_ie && !is_pb && !is_undefined(scrollMagicController)) {
			$(".znParallax-background").each(function(index, el) {
				...
				var $el = $(el),
					$bg_el = $el.find('.kl-bg-source__bgimage, .kl-bg-source__video, .kl-bg-source__iframe-wrapper, .slide-item-bg, .kl-video-container');

					var bgtween = TweenLite.to($bg_el[0], 1, {
							y: "80%",
					...
					new ScrollMagic.Scene({triggerElement: $el[0], triggerHook: "onEnter", duration: "200%"})
						.setTween(bgtween)
						.addTo(scrollMagicController);
			});
		}
