			...
			if ( !empty( $option['std'] ) && !empty ( $option['supports'] ) && $option['supports'] == 'block' ) {
				...
				if(isset($option['dragup'])){
					...
					if(is_array($option['dragup']) && !empty($option['dragup']) ){
						$attrs .= isset($option['dragup']['no-unit']) && $option['dragup']['no-unit'] ? ' data-no-unit' : '';
						$attrs .= isset($option['dragup']['unit']) && $option['dragup']['unit'] ? ' data-dragkey-unit="'.$option['dragup']['unit'].'"' : '';
		...
		function smart_slider($option) {
			...
			if(isset($option['std']) && !empty($option['std']) ){
				...
				if($support_breakpoints) {
					...
					$std['unit_md'] = isset($option['std']['unit_md']) && !empty($option['std']['unit_md']) ? $option['std']['unit_md'] : 'px';

					$std['sm'] = isset($option['std']['sm']) && $option['std']['sm'] != '' ? $option['std']['sm'] : '';
					$std['unit_sm'] = isset($option['std']['unit_sm']) && !empty($option['std']['unit_sm']) ? $option['std']['unit_sm'] : 'px';

					$std['xs'] = isset($option['std']['xs']) && $option['std']['xs'] != '' ? $option['std']['xs'] : '';
					$std['unit_xs'] = isset($option['std']['unit_xs']) && !empty($option['std']['unit_xs']) ? $option['std']['unit_xs'] : 'px';
				}
				if($support_props){
			...
			$output  = '<div class="zn_slider znSmartSlider clearfix">';
				...
				$output .= '<input type="text" class="zn_input zn_input_text znSmartSlider-input znSmartSlider-mainInput wp-slider-input " '.$is_live.' min="'.$min.'" step="'.$step.'" max="'.$max.'" name="'.$option['id'].'[lg]" value="'.$std['lg'].'" '.(isset($option['disabled_slider']) ? 'readonly':'').' >';
