	...
	$.ZnHtml.prototype = {
		...
		dragKeyFields : function(scope){
			...
			field.each(function(index, el) {
				...
				if($el.is('[data-dragkey-min]') && $el.attr('data-dragkey-min') !== '')
					$opt.min = $el.attr('data-dragkey-min');

				if($el.is('[data-dragkey-max]') && $el.attr('data-dragkey-max') !== '')
		...
		if(def.drag){
			this.on( 'mousedown', function(e){
				...
				$(document).on( 'mousemove', {
					...
					var offset_sum = e.data.cur-offset;
					if( offset_sum < def.min || offset_sum > def.max ) return;
