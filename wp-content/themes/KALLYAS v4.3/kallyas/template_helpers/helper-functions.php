	...
	class WpkPageHelper
	{
		...
		public static function zn_background_source( $args = array() )
		{
			...
			if ( $sourceType ):
				...
				if ( $args['enable_parallax'] == 'yes' && !ZNPB()->is_active_editor ) {
				...
				if ( $sourceType == 'image' ) {
					...
					if ( !empty( $background_image ) ) {
						$bg_source .= '<div class="kl-bg-source__bgimage" style="' . implode( ';', $background_styles ) . '">';
						$bg_source .= '</div>';
					}
			...
			if ( $args['source_overlay'] != 0 ) {
				...
				$bg_source .= '<div class="kl-bg-source__overlay '.$extraclass.'" style="' . $ovstyle . '"></div>';
			...
			if ( $bg_source != '' ) {
				echo '<div class="kl-bg-source '.$bg_container_class.'">' . $bg_source . '</div>';
			}
