...
global $zn_config;
	...
	if($zn_sp_linkto_external == 'yes'){
		$sp_link_ext = zn_extract_link( $sp_link, 'portfolio-item-link hoverLink' );
			...
			if ( ! empty ( $port_media ) && is_array( $port_media ) ) {
				...
				elseif ( ! empty( $saved_image ) ) {
					...
					if ( $zn_link_portfolio == 'yes' ) {
						$ptf_item_link = '<a href="' . get_permalink() . '" class="portfolio-item-link hoverLink"></a>';
					}
					else {
						$ptf_item_link = '<a href="' . $saved_image . '" data-type="image" data-lightbox="image" class="portfolio-item-link hoverLink"></a>';
					}
