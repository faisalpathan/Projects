...
// Check slider status.
add_filter( 'onefold_filter_slider_status', 'onefold_check_slider_status' );

// Add slider to the theme.
add_action( 'onefold_action_before_content', 'onefold_add_featured_slider', 5 );

// Slider details.
add_filter( 'onefold_filter_slider_details', 'onefold_get_slider_details' );


if ( ! function_exists( 'onefold_get_slider_details' ) ) :
	/**
	 * Slider details.
	 *
	 * @since 1.0.0
	 ...
	 */
	function onefold_get_slider_details( $input ) {

		$featured_slider_type           = onefold_get_option( 'featured_slider_type' );
		$featured_slider_number         = onefold_get_option( 'featured_slider_number' );
		$featured_slider_read_more_text = onefold_get_option( 'featured_slider_read_more_text' );
		...
		switch ( $featured_slider_type ) {
			...
			case 'featured-page':
				...
				for ( $i = 1; $i <= $featured_slider_number ; $i++ ) {
					$id = onefold_get_option( 'featured_slider_page_' . $i );
					if ( ! empty( $id ) ) {
						$ids[] = absint( $id );
					}
				}
				...
				if ( ! empty( $all_posts ) ) {
					...
					foreach ( $all_posts as $key => $post ) {
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							$image_array = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'onefold-slider' );
							$slides[ $cnt ]['images']  = $image_array;
							$slides[ $cnt ]['title']   = esc_html( $post->post_title );
							$slides[ $cnt ]['url']     = esc_url( get_permalink( $post->ID ) );
							$slides[ $cnt ]['excerpt'] = onefold_the_excerpt( apply_filters( 'onefold_filter_slider_caption_length', 20 ), $post );
...
if ( ! function_exists( 'onefold_add_featured_slider' ) ) :
	/**
	 * Add featured slider.
	 *
	 * @since 1.0.0
	 */
	function onefold_add_featured_slider() {
		...
		$slider_details = apply_filters( 'onefold_filter_slider_details', $slider_details );
		...
		// Render slider now.
		onefold_render_featured_slider( $slider_details );

	}
endif;


if ( ! function_exists( 'onefold_render_featured_slider' ) ) :
	/**
	 ...
	 */
	function onefold_render_featured_slider( $slider_details = array() ) {

		if ( empty( $slider_details ) ) {
			return;
		}
		$featured_slider_transition_effect   = onefold_get_option( 'featured_slider_transition_effect' );
		$featured_slider_enable_caption      = onefold_get_option( 'featured_slider_enable_caption' );
		$featured_slider_enable_arrow        = onefold_get_option( 'featured_slider_enable_arrow' );
		$featured_slider_enable_pager        = onefold_get_option( 'featured_slider_enable_pager' );
		$featured_slider_enable_autoplay     = onefold_get_option( 'featured_slider_enable_autoplay' );
		$featured_slider_transition_duration = onefold_get_option( 'featured_slider_transition_duration' );
		$featured_slider_transition_delay    = onefold_get_option( 'featured_slider_transition_delay' );
...
if( ! function_exists( 'onefold_check_slider_status' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function onefold_check_slider_status( $input ) {

		// Slider status.
		$featured_slider_status = onefold_get_option( 'featured_slider_status' );
